...
void CInode::decode_store(bufferlist::iterator& bl) {
  ...
  if ((struct_v == 2 || struct_v == 3) && inode.is_dir()) {
    bool default_layout_exists;
    ::decode(default_layout_exists, bl);
    if (default_layout_exists) {
      /* the on-disk thing is either a default_file_layout or a
	 file_layout_policy_t, but we only care about the layout portion
	 of that */
      __u8 layout_struct_v;
      ::decode(layout_struct_v, bl);
      if (layout_struct_v > 1) { // a file_layout_policy_t, with new encoding
	__u32 tmp;
	::decode(layout_struct_v, bl); // compat_v, really
	::decode(tmp, bl); // struct_len
      }
